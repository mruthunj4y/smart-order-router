...
describe('portion provider', () => {
  ...
  describe('getPortion test', () => {
    describe('exact in quote test', () => {
      GREENLIST_TOKEN_PAIRS.forEach((pair) => {
        const token1: Currency | Token = pair[0].isNative
          ? (pair[0] as Currency)
          : pair[0].wrapped;
        const token2: Currency | Token = pair[1].isNative
          ? (pair[1] as Currency)
          : pair[1].wrapped;
      ...
      async function exactInGetPortionAndAssert(token2: Currency | Token) {
        const quoteAmount = parseAmount(expectedQuote, token2);
        const quoteGasAdjustedAmount = quoteAmount.subtract(
          parseAmount(expectedGas, token2)
        );
        ...
        const expectedPortionAmount = quoteAmount.multiply(
          new Fraction(expectedPortion.bips, 10_000)
        );
        expect(portionAmount?.quotient.toString()).toBe(
          expectedPortionAmount.quotient.toString()
        );

        // (1605.56 - 2.35) * 10^8 - 80278000 = 160240722000
        const expectedQuoteGasAndPortionAdjusted =
          quoteGasAdjustedAmount.subtract(expectedPortionAmount);
        expect(portionAdjustedQuote?.quotient.toString()).toBe(
          expectedQuoteGasAndPortionAdjusted.quotient.toString()
        );

        // 160240722000 / 10^8 = 1602.40722000
        expect(portionAdjustedQuote?.toExact()).toBe(
          expectedQuoteGasAndPortionAdjusted.toExact()
        );
      }
    });
    ...
    describe('exact out quote test', () => {
      ...
      async function exactOutGetPortionAndAssert(
        ...
        const quoteGasAdjustedAmount = quoteAmount.add(
          parseAmount(expectedGas, token1)
        );

        const expectedPortionAmount = amount.multiply(
          new Fraction(expectedPortion.bips, 10_000)
        );
        const swapConfig: SwapOptions = {
          type: SwapType.UNIVERSAL_ROUTER,
        ...
        const expectedPortionQuoteAmount = portionAmount!
          .divide(portionAmount!.add(amount))
          .multiply(quoteAmount);
        expect(actualPortionQuoteAmount!.quotient.toString()).toBe(
          expectedPortionQuoteAmount.quotient.toString()
        );
        ...
        const expectedCorrectedQuoteAmount = quoteAmount.subtract(
          actualPortionQuoteAmount!
        );
        expect(actualCorrectedQuoteAmount?.quotient.toString()).toBe(
          expectedCorrectedQuoteAmount.quotient.toString()
        );

        const actualCorrectedQuoteGasAdjustedAmount =
          portionProvider.getQuoteGasAdjusted(
            TradeType.EXACT_OUTPUT,
            quoteGasAdjustedAmount,
            actualPortionQuoteAmount
          );
        const expectedCorrectedQuoteGasAdjustedAmount =
          quoteGasAdjustedAmount.subtract(actualPortionQuoteAmount!);
        expect(actualCorrectedQuoteGasAdjustedAmount?.quotient.toString()).toBe(
          expectedCorrectedQuoteGasAdjustedAmount.quotient.toString()
        );

        const actualCorrectedQuoteGasAndPortionAdjustedAmount =
          portionProvider.getQuoteGasAndPortionAdjusted(
            TradeType.EXACT_OUTPUT,
        ...
        expect(
          actualCorrectedQuoteGasAndPortionAdjustedAmount?.quotient.toString()
    ...
    describe('TokenOutHasFot flag is true', () => {
      GREENLIST_TOKEN_PAIRS.forEach((pair) => {
        ...
        it(`token address ${tokenAddress1} to token address ${tokenAddress2} within the list, but tokenOut has FOT with external transfer failed, should not have portion`, async () => {
          const quoteAmount = parseAmount(expectedQuote, token2);
        ...
        it(`token address ${tokenAddress1} to token address ${tokenAddress2} within the list, but tokenOut has FOT with double fee, should not have portion`, async () => {
          const quoteAmount = parseAmount(expectedQuote, token2);
  ...
  describe('getRouteWithQuotePortionAdjusted test', () => {
    it('exact in test', () => {
      ...
      const routesWithQuotePortionAdjusted =
      ...
      routesWithQuotePortionAdjusted.forEach(
        (routeWithQuotePortionAdjusted) => {
          if (routeWithQuotePortionAdjusted instanceof V2RouteWithValidQuote) {
            expect(
              routeWithQuotePortionAdjusted.quote.quotient.toString()
            ).toEqual(
              oneHundredPercent
                .subtract(new Percent(FLAT_PORTION.bips, 10_000))
                .multiply(20)
                .quotient.toString()
            );
          }
          ...
          if (
            routeWithQuotePortionAdjusted instanceof MixedRouteWithValidQuote
          ) {
            expect(
              routeWithQuotePortionAdjusted.quote.quotient.toString()
            ).toEqual(
