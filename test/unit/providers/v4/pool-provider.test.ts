...
import { ADDRESS_ZERO, FeeAmount } from '@surge/v3-sdk';
...
describe('PoolProvider', () => {
  it('can fetch pool state', async () => {
    ...
    const currencyPairs: Array<[Currency, Currency, number, number, string]> =
      new Array([USDC_SEPOLIA, OP_SEPOLIA, FeeAmount.LOW, 10, ADDRESS_ZERO]);
    const poolAccessor = await poolProvider.getPools(currencyPairs, {
      blockNumber: 6534459,
    });
    poolAccessor.getAllPools().forEach((pool) => {
      expect(pool.poolId).toEqual(
        '0xa40318dea5fabf21971f683f641b54d6d7d86f5b083cd6f0af9332c5c7a9ec06'
      );
