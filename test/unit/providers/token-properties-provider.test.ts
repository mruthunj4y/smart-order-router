import { ChainId, Token, WETH9 } from '@surge/sdk-core';
...
describe('TokenPropertiesProvider', () => {
  ...
  describe('get token fees by address', () => {
    it('succeeds to get token fee and updates cache', async () => {
      const underlyingCache: NodeCache = new NodeCache({
        stdTTL: 3600,
        useClones: false,
      });
      const tokenPropertiesResultCache: NodeJSCache<TokenPropertiesResult> =
        new NodeJSCache(underlyingCache);
      ...
      expect(
        await tokenPropertiesResultCache.get(
          CACHE_KEY(ChainId.MAINNET, token.address.toLowerCase())
        )
      ).toBeUndefined();
      const tokenPropertiesMap =
        await tokenPropertiesProvider.getTokensProperties([token], {
          enableFeeOnTransferFeeFetching: true,
        });
      expect(tokenPropertiesMap[token.address.toLowerCase()]).toBeDefined();
      assertExpectedTokenProperties(
        tokenPropertiesMap[token.address.toLowerCase()],
        BigNumber.from(213),
        BigNumber.from(800),
        TokenValidationResult.FOT
      );

      const cachedTokenProperties = await tokenPropertiesResultCache.get(
        CACHE_KEY(ChainId.MAINNET, token.address.toLowerCase())
      );
      expect(cachedTokenProperties).toBeDefined();
      assertExpectedTokenProperties(
        cachedTokenProperties,
        BigNumber.from(213),
        BigNumber.from(800),
        TokenValidationResult.FOT
      );

      underlyingCache.getTtl(
        CACHE_KEY(ChainId.MAINNET, token.address.toLowerCase())
      );
      expect(
        Math.floor(
      ...
      ).toEqual(currentEpochTimeInSeconds + POSITIVE_CACHE_ENTRY_TTL);
    });
    ...
    it('succeeds to get token fee cache hit and second token fee fetcher call is skipped', async function () {
      ...
      const tokenPropertiesMapFromSecondCall =
        await tokenPropertiesProvider.getTokensProperties([token], {
          enableFeeOnTransferFeeFetching: true,
        });
      assertExpectedTokenProperties(
        tokenPropertiesMapFromSecondCall[token.address.toLowerCase()],
        BigNumber.from(213),
        BigNumber.from(800),
        TokenValidationResult.FOT
      );
    ...
    it('succeeds to get token allowlist with no on-chain calls nor caching', async function () {
      ...
      const allowListToken = new Token(
        1,
        '0x777E2ae845272a2F540ebf6a3D03734A5a8f618e',
        18
      );
      const tokenPropertiesMap =
        await tokenPropertiesProvider.getTokensProperties([allowListToken], {
          enableFeeOnTransferFeeFetching: true,
        });

      expect(
        tokenPropertiesMap[allowListToken.address.toLowerCase()]
      ).toBeDefined();
      expect(
        tokenPropertiesMap[allowListToken.address.toLowerCase()]?.tokenFeeResult
      ).toBeUndefined();
      assertExpectedTokenProperties(
        tokenPropertiesMap[allowListToken.address.toLowerCase()],
        undefined,
      ...
      expect(
        await tokenPropertiesResultCache.get(
          CACHE_KEY(ChainId.MAINNET, allowListToken.address.toLowerCase())
        )
      ).toBeUndefined();
    });
    ...
    it('succeeds to get token properties in a single batch', async function () {
      ...
      const tokenPropertiesMap =
        await tokenPropertiesProvider.getTokensProperties(tokens, {
          enableFeeOnTransferFeeFetching: true,
        });
      ...
      for (const token of tokens) {
        ...
        const expectedBuyFeeBps =
          tokenPropertiesMap[address]?.tokenFeeResult?.buyFeeBps;
        const expectedSellFeeBps =
          tokenPropertiesMap[address]?.tokenFeeResult?.sellFeeBps;
        assertExpectedTokenProperties(
          tokenPropertiesMap[address],
          expectedBuyFeeBps,
          expectedSellFeeBps,
          TokenValidationResult.FOT
        );
    ...
    it('all token fee fetch failed', async function () {
      ...
      mockTokenFeeFetcher.fetchFees
        .withArgs(tokens.map((token) => token.address))
        .throws(new Error('Failed to fetch fees for token 1'));
      ...
      for (const token of tokens) {
        ...
        expect(
          tokenPropertiesMap[address]?.tokenValidationResult
        ).toBeUndefined();
        ...
        ).toEqual(currentEpochTimeInSeconds + NEGATIVE_CACHE_ENTRY_TTL);
      }
    });
    ...
    it('real ETH and BITBOY token fee fetch, only BITBOY fetched', async function () {
      ...
      expect(
        tokenPropertiesMap[WETH9[ChainId.MAINNET]!.address.toLowerCase()]
      ).toBeDefined();
      expect(
        tokenPropertiesMap[WETH9[ChainId.MAINNET]!.address.toLowerCase()]
          ?.tokenFeeResult
      ...
      expect(
        tokenPropertiesMap[BITBOY.address.toLowerCase()]?.tokenFeeResult
      ).toBeDefined();
      expect(
        tokenPropertiesMap[BITBOY.address.toLowerCase()]?.tokenValidationResult
      ).toBeDefined();
      assertExpectedTokenProperties(
        tokenPropertiesMap[BITBOY.address.toLowerCase()],
        BITBOY?.buyFeeBps,
        BITBOY?.sellFeeBps,
        TokenValidationResult.FOT
      );
      ...
      const cachedTokenProperties = await tokenPropertiesResultCache.get(
        CACHE_KEY(ChainId.MAINNET, BITBOY.address.toLowerCase())
      );
      expect(cachedTokenProperties).toBeDefined();
      assertExpectedTokenProperties(
        cachedTokenProperties,
      ...
      expect(
        ...
        currentEpochTimeInSeconds + POSITIVE_CACHE_ENTRY_TTL
      );
      expect(
        Math.floor(
          (underlyingCache.getTtl(
  ...
  function assertExpectedTokenProperties(
    ...
    if (expectedBuyFeeBps) {
      expect(
        tokenProperties?.tokenFeeResult?.buyFeeBps?.eq(expectedBuyFeeBps)
      ).toBeTruthy();
    } else {
    ...
    if (expectedSellFeeBps) {
      expect(
        tokenProperties?.tokenFeeResult?.sellFeeBps?.eq(expectedSellFeeBps)
      ).toBeTruthy();
    } else {
      expect(tokenProperties?.tokenFeeResult?.sellFeeBps).toBeUndefined();
    }

    expect(tokenProperties?.tokenValidationResult).toEqual(
      expectedTokenValidationResult
    );
  }
});
